...
static func create(p_hero: Hero, p_enemy: Enemy, p_location_id: String = "", p_monster: Monster = null) -> BattleConfig:
	var cfg := BattleConfig.new()
	cfg.hero = p_hero
	cfg.monster_id = p_enemy.monster_id
	cfg.spawn_point_id = p_enemy.spawn_point_id
	cfg.location_id = p_location_id
